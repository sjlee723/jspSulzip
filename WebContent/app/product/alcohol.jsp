...
<c:forEach var="alcohol" items="${alcoholList}" varStatus="idx">
	<c:if test="${idx.count % 3 == 1}">
					<div class="card-group">
	</c:if>
					<div class="card" data-num="${alcohol.getProductNumber()}">
                        <img src="${pageContext.request.contextPath}/assets/img/alcohol/${alcohol.getProductSystemName()}" class="card-img-top inside">
                        <div class="card-body">
                            <p class="card-text-1">술</p>
                            <h5 class="card-title inside">${alcohol.getProductNameKor()}</h5>
                            <p class="card-text-2"><small class="text-muted"><fmt:formatNumber value="${alcohol.getProductPrice()}" pattern="#,###"/>
